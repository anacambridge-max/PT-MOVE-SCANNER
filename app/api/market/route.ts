...
export async function GET(){
 ...
 try{
  const all=await instrumentMaster(),today=date(),from=date(-30),to=date(-1)
  ...
  const diagnostics={universe:stocks.length,quoteMatched:universe.length,cprPass:0,dailyHighPass:0,barsMatched:0,volumePass:0,breakoutPass:0,finalPass:0}
  const rows=await mapLimit(universe,CONCURRENCY,async({item,cashKey,futureKey,cashQuote,futureQuote,ohlc})=>{try{
    ...
    diagnostics.cprPass++
    const [futureToday,futureHistory,cashToday]=await Promise.all([intraday(futureKey,token),historical(futureKey,token,to,from),intraday(cashKey,token)])
    const fBars=dayBars(futureToday,today),fHist=futureHistory.filter(c=>day(c)<today).sort((a,b)=>ts(a)-ts(b)),cBars=dayBars(cashToday,today)
    if(!fBars.length||!cBars.length)return null
    diagnostics.barsMatched++
    ...
    const cashByBucket=new Map<number,C>();for(const c of cBars)cashByBucket.set(bucket(ts(c)),c)
    let signal:any=null
    for(const f of fBars){
      ...
      if(!breakoutHigh&&!breakoutLow)continue
      diagnostics.breakoutPass++
      const direction=breakoutHigh?'LONG':'SHORT'
      signal={time:f[0],direction,volume:fv,sma20,rvol:sma20?fv/sma20:0,breakout:breakoutHigh?'PDH BREAK':'PDL BREAK'}
    }
    if(!signal)return null
    diagnostics.finalPass++
    return {rank:0,symbol:item.underlying_symbol!.toUpperCase(),name:item.name||item.trading_symbol||item.underlying_symbol!,bias:signal.direction,change:Number(cashQuote.net_change??0),lastPrice:cashQuote.last_price,futurePrice:futureQuote.last_price,rvol:+signal.rvol.toFixed(2),volume:signal.volume,avgVolume20:+signal.sma20.toFixed(0),breakout:signal.breakout,signalTime:signal.time,score:100,setup:`5M FUT VOL > 2× SMA20 + ${signal.breakout} + NARROW CPR`,cpr,cprWidth:+cpr.pct.toFixed(3),dailyHigh:+currentDailyHigh.toFixed(2),prevDayHigh:prevH,prevDayLow:prevL,conditions:{futuresVolume:true,cashBreakout:true,dailyHighAbove50:true,narrowCPR:true}}
   }catch{return null}})
  const candidates=rows.filter(Boolean).sort((a:any,b:any)=>ts([b.signalTime,0,0,0,0,0,0])-ts([a.signalTime,0,0,0,0,0,0])).slice(0,TOP).map((x:any,i)=>({...x,rank:i+1}))
