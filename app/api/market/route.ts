...
const MAX_SCAN=40,CONCURRENCY=12,TOP=30
...
async function upstoxJson(url:string,t:string,label:string){let last=0;for(let attempt=0;attempt<3;attempt++){const r=await fetch(url,{headers:{Accept:'application/json',Authorization:`Bearer ${t}`},cache:'no-store'});if(r.ok)return await r.json();last=r.status;if(r.status!==429)throw Error(`${label} failed: ${r.status}`);const retry=Number(r.headers.get('retry-after')||0);await new Promise(resolve=>setTimeout(resolve,retry>0?Math.min(retry*1000,2500):400*(attempt+1)))}throw Error(`${label} failed: ${last} after retries`)}
...
async function prevDaily(k:string,t:string){const d=await upstoxJson(`${V3}/historical-candle/${encodeURIComponent(k)}/days/1/${date(-1)}/${date(-7)}`,t,'Previous daily');return(d.data?.candles??[]) as C[]}
function trigger(session:C[],prev:D){if(session.length<21)return null;const candles=[...session].sort((a,b)=>new Date(a[0]).getTime()-new Date(b[0]).getTime()),opening=candles.slice(0,3),openingHigh=Math.max(...opening.map(x=>+x[2])),openingLow=Math.min(...opening.map(x=>+x[3]));let best:any=null;for(let i=20;i<candles.length;i++){const c=candles[i],v=+c[5]||0,sma=avg(candles.slice(i-20,i).map(x=>+x[5]||0)),vr=sma?v/sma:0;if(vr<2.5)continue;const h=+c[2],lo=+c[3],cl=+c[4],pdh=h>prev.high,pdl=lo<prev.low;if(!pdh&&!pdl)continue;const direction=(pdh?'LONG':'SHORT') as 'LONG'|'SHORT',orb=direction==='LONG'?cl>openingHigh:cl<openingLow;const hit={vr,pdh,pdl,orb,direction,candleTime:c[0]};if(!best||((orb?10:0)+vr)>((best.orb?10:0)+best.vr))best=hit}return best}
...
export async function GET(){
 ...
 try{
  ...
  const keys=stocks.map(x=>x.underlying_key!).concat(ix.map(x=>x.instrument?.instrument_key).filter(Boolean) as string[])
  const qs=await quotes(keys,token)
  ...
  const pool=stocks.map(item=>({item,key:key(item.underlying_key),quote:qs[key(item.underlying_key)]})).filter((x):x is {item:I;key:string;quote:Q}=>Boolean(x.quote&&x.quote.last_price>0)).sort((a,b)=>Math.abs(b.quote.net_change??0)-Math.abs(a.quote.net_change??0)||(b.quote.volume??0)-(a.quote.volume??0)).slice(0,MAX_SCAN)
  ...
  const rows=await map(pool,CONCURRENCY,async({item,key,quote})=>{
   try{
    const [ics,dcs]=await Promise.all([intraday5m(key,token),prevDaily(key,token)])
    const session=ics.filter(c=>dayOf(c)===date()),ds=daily(dcs),prev=ds.filter(x=>x.date<date()).at(-1)
    if(!session.length||!prev){intradayErrors++;return null}
    intradaySuccess++
    const f=trigger(session,prev)
    if(!f)return null
    triggerCount++
    ...
    const recent=ds.slice(-7),pr=prev.high-prev.low,nr4=recent.length>=4&&pr<=Math.min(...recent.slice(-4).map(x=>x.high-x.low)),nr7=recent.length>=7&&pr<=Math.min(...recent.slice(-7).map(x=>x.high-x.low))
    let score=55+Math.min(18,f.vr*5)+(f.orb?7:0)+(rsAligned?7:0)+(marketAligned?5:0)+(nr4?4:0)+(nr7?4:0);score=Math.min(99,Math.round(score))
    const confirmation=['EXT VOL',f.orb?'ORB':'',rsAligned?'RS':'',marketAligned?'MKT':''].filter(Boolean).join(' ')
    return{rank:0,symbol:item.underlying_symbol!.toUpperCase(),name:item.name||item.trading_symbol||item.underlying_symbol!,sector:'F&O',score,bias:f.direction,change,volume:(quote.volume??0).toLocaleString('en-IN'),rs:rsAligned?'Strong':'Neutral',setup:`${confirmation} + ${f.pdh?'PDH BREAK':'PDL BREAK'}${nr4?' + NR4':''}${nr7?' + NR7':''}`,confidence:score>=90?'A+':score>=80?'A':score>=70?'B+':'B',lastPrice:quote.last_price,oi:quote.oi??null,rvol:+f.vr.toFixed(2),rangePct:0,nr4,nr7,pdBreak:f.pdh?'PDH':'PDL',t1Date:date(),filterPass:true,filterLabel:'LIVE NSE F&O + V3 current-day 5m + previous-day PDH/PDL + extreme volume ≥2.5× previous 20 candles',filterDate:date(),signalTime:f.candleTime,marketDirection,marketAligned,openingRangeBreak:f.orb,extremeVolume:true}
   }catch{return null}
  })
  const candidates=rows.filter((x):x is NonNullable<typeof x>=>Boolean(x)).sort((a,b)=>b.score-a.score||Math.abs(b.change)-Math.abs(a.change)).slice(0,TOP).map((x,i)=>({...x,rank:i+1}))
  ...
  return NextResponse.json({ok:true,source:'UPSTOX LIVE + V3 INTRADAY 5M',timestamp:new Date().toISOString(),universeCount,scanned:pool.length,expiry:nearExpiry,diagnostics:{intradaySuccess,intradayErrors,triggerCount,candidates:candidates.length,marketHours,marketDirection,niftyChange},filter:{universe:`NSE F&O stock futures — top ${MAX_SCAN} live movers scanned`,fiveMinuteVolume:'Current-day 5m trigger volume ≥ 2.5 × previous 20-candle SMA — mandatory',breakout:'5m High > Previous Day High for LONG OR 5m Low < Previous Day Low for SHORT — mandatory',openingRange:'Breakout candle close above first 15m high / below first 15m low gets confirmation bonus',relativeStrength:'LONG outperforming NIFTY 50 by 0.25% or SHORT underperforming by 0.25% is confirmation only',marketDirection:'NIFTY 50 direction is confirmation, not a hard blocker',structure:'Previous-day NR4/NR7 confirmation',price:'No artificial price filter'},candidates,indexes:indexesOut})
