...
export async function GET(){
 ...
 try{
  ...
  // FAST PATH: bulk-filter by live price vs PDH/PDL BEFORE making any 5M requests.
  const scanPool=universe.filter(x=>{const p=x.quote.last_price,h=x.ohlc?.prev_ohlc?.high,l=x.ohlc?.prev_ohlc?.low;return Number.isFinite(h)&&Number.isFinite(l)&&(p>h!||p<l!)})
  ...
  return NextResponse.json({ok:true,source:'UPSTOX LIVE + FAST BULK PDH/PDL PRE-FILTER + 5M EXTREME-VOLUME ENGINE',timestamp:new Date().toISOString(),universeCount:stocks.length,scanned:universe.length,expiry:near,diagnostics:{intradaySuccess,intradayErrors,triggerCount,scanPool:scanPool.length,candidates:candidates.length,liveFallbackChecks,liveFallbackHits,pdhSourceCounts:{UPSTOX_PREV_OHLC:scanPool.length,HISTORICAL_5M:0,FAILED:universe.length-scanPool.length},preFilter:`Bulk price/PDH/PDL filter reduced ${universe.length} stocks to ${scanPool.length} before 5M requests`,marketHours:true,marketDirection,niftyChange},filter:{universe:'NSE F&O stock futures — current near-expiry underlyings',fiveMinuteVolume:'5m candle volume ≥ 2.5 × previous 20 completed 5m candles — mandatory',breakout:'5m candle close/current price > previous-session HIGH (PDH) for LONG OR < previous-session LOW (PDL) for SHORT — mandatory',live:'Forming 5m candle is checked from 1m data only after PDH/PDL bulk pre-filter passes'},indexes,candidates})
