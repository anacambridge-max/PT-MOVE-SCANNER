...
export async function GET(){
 ...
 try{
  ...
  const rows=await limit(candidatesCash,CONCURRENCY,async x=>{try{
   ...
   const allF=[...hRaw.filter(c=>day(c)!==today),...fb].sort((a,b)=>t(a)-t(b));const fBy=new Map(fb.map(c=>[t(c),c]));const qualified:any[]=[]
   ...
   diagnostics.volumePass+=qualified.length;diagnostics.finalPass++;const hit=qualified[qualified.length-1]
