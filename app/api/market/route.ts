...
const key = (x?: string | null) => x?.trim().replace(/\|/g, ':').toUpperCase() ?? ''
const symbolKey = (x?: string | null) => x?.trim().toUpperCase() ?? ''
...
function expiry(x?: number | string) {
  if (typeof x === 'string') { const z = Date.parse(x); return Number.isFinite(z) ? new Date(z).toISOString().slice(0, 10) : '' }
...
async function api(url: string, token: string, label: string) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    await new Promise(resolve => setTimeout(resolve, retry > 0 ? Math.min(retry * 1000, 5000) : 300 * (i + 1)))
  }
...
// Index every quote by BOTH Upstox's response key and the returned instrument_token.
// Upstox responses use display keys such as NSE_EQ:NHPC while instrument_token is NSE_EQ|INE....
async function quotes(keys: string[], token: string) {
  const out: Record<string, Q> = {}
  for (let i = 0; i < keys.length; i += 400) {
    const requested = keys.slice(i, i + 400)
    const u = new URL(`${V2}/market-quote/quotes`)
    u.searchParams.set('instrument_key', requested.join(','))
    const d = (await api(u.toString(), token, 'Quotes')).data ?? {}
    for (const [responseKey, q] of Object.entries(d) as [string, Q][]) {
      if (responseKey) out[key(responseKey)] = q
      if (q.instrument_token) out[key(q.instrument_token)] = q
      if (q.symbol) out[symbolKey(q.symbol)] = q
    }
  }
  return out
}

async function daily(keys: string[], token: string) {
  ...
  for (let i = 0; i < keys.length; i += 400) {
    ...
    for (const [responseKey, q] of Object.entries(d) as [string, O][]) {
      if (responseKey) out[key(responseKey)] = q
      if (q.instrument_token) out[key(q.instrument_token)] = q
      if (q.symbol) out[symbolKey(q.symbol)] = q
    }
...
async function intra(k: string, token: string) { const d = await api(`${V3}/historical-candle/intraday/${encodeURIComponent(k)}/minutes/5`, token, '5M intraday'); return (d.data?.candles ?? []) as C[] }
async function hist(k: string, token: string) { const d = await api(`${V3}/historical-candle/${encodeURIComponent(k)}/minutes/5/${date(-1)}/${date(-7)}`, token, '5M history'); return (d.data?.candles ?? []) as C[] }
...
export async function GET() {
  ...
  try {
    ...
    for (const x of fut.filter(x => expiry(x.expiry) === near)) by.set(symbolKey(x.underlying_symbol), x)
    const stocks = [...by.values()].slice(0, MAX_STOCKS)
    ...
    let cashQuoteMatched = 0, futureQuoteMatched = 0, previousDayMatched = 0
    const universe = stocks.map(item => {
      const cash = cq[key(item.underlying_key)] || cq[symbolKey(item.underlying_symbol)]
      const future = fq[key(item.instrument_key)] || fq[symbolKey(item.trading_symbol)]
      const prev = pd[key(item.underlying_key)]?.prev_ohlc || pd[symbolKey(item.underlying_symbol)]?.prev_ohlc
      if (cash?.last_price > 0) cashQuoteMatched++
      if (future?.last_price > 0) futureQuoteMatched++
      if (prev) previousDayMatched++
      return { item, ck: item.underlying_key!, fk: item.instrument_key, cash, future, prev }
    }).filter(x => x.cash?.last_price > 0 && x.future?.last_price > 0 && x.prev)

    const diagnostics = { universe: stocks.length, cashQuoteMatched, futureQuoteMatched, quoteMatched: universe.length, previousDayMatched, cashBars: 0, futuresBars: 0, dailyHighPass: 0, cashBreakoutPass: 0, volumePass: 0, finalPass: 0, errors: 0 }
    ...
    const rows = await limit(candidatesCash, CONCURRENCY, async x => {
      try {
        ...
        const fBy = new Map(fb.map(c => [ts(c), c])), qualified: any[] = []
        for (const h of x.hits) {
          const f = fBy.get(ts(h.c)); if (!f) continue
          const idx = allF.findIndex(c => ts(c) === ts(f)); if (idx < 20) continue
          // Chartink-style SMA(Volume,20): include the current candle in the 20-bar SMA.
          const window = allF.slice(idx - 19, idx + 1).map(c => Number(c[5]) || 0)
          const sma = avg(window), vol = Number(f[5]) || 0
    ...
    const indexes = ik.map((k, i) => { const q = iq[key(k)] || iq[symbolKey(labels[i])]; const ch = q?.last_price && q.net_change != null ? q.net_change / (q.last_price - q.net_change) * 100 : null; return { title: labels[i], value: q?.last_price ?? null, change: ch } })
